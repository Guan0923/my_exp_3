__all__ = ["PatchTST_backbone_with_Sequence"]
...
# Cell
class PatchTST_backbone_with_Sequence(nn.Module):
    def __init__(
        self,
        c_in: int,
        k: int,
        ...
        **kwargs,
        ...
        if create_method == "Linear":
            ...
            nn.init.kaiming_normal_(self.create_s)
            self.create_s = nn.Parameter(self.create_s, requires_grad=True)
        else:
            raise ValueError("create_method must be Linear, Conv1d, or init")
    ...
    def forward(self, z):  # z: [bs x nvars x seq_len]
        ...
        # model
        z = self.backbone(z)  # z: [bs x nvars x d_model x patch_num + k]
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num + k]

        n_vars = x.shape[1]
        # Input encoding
        x = x.permute(0, 1, 3, 2)  # x: [bs x nvars x patch_num + k x patch_len]
        x = self.W_P(x)  # x: [bs x nvars x patch_num + k x d_model]
...
class _MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        )  # k_s    : [bs x n_heads x d_k x q_len + k] - transpose(1,2) + transpose(2,3)
        v_s = (
            self.W_V(V).view(bs, -1, self.n_heads, self.d_v).transpose(1, 2)
        )  # v_s    : [bs x n_heads x q_len + k x d_v]
...
class _ScaledDotProductAttention(nn.Module):
    ...
    def forward(
        ...
        Input shape:
            ...
            v               : [bs x n_heads x seq_len + k x d_v]
        ...
        )  # attn_scores : [bs x n_heads x max_q_len x q_len + k]
        ...
        ):  # attn_mask with shape [q_len x seq_len + k] - only used when q_len == seq_len
        ...
        attn_weights = F.softmax(
            attn_scores, dim=-1
        )  # attn_weights   : [bs x n_heads x max_q_len x q_len + k]
